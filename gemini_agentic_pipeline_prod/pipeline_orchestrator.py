...
async def generate_social_media_posts_pipeline(
    ...
    for entry in platforms_post_types_map:
        if isinstance(entry, dict) and len(entry) == 1:
            ...
            print(f"⚠️ Warning: Invalid entry in platforms_post_types_map: {entry}. Skipping.")
            continue

    if not platform_configs:
        error_msg = "No valid platforms configured in platforms_post_types_map."
        print(f"🚫 Error: {error_msg} Aborting.")
        return {
            "error": error_msg,
    ...
    print(f"🗺️ Targeting platforms based on PLATFORMS_POST_TYPES_MAP: {', '.join(target_platforms_from_map)}")
    ...
    print(f"📝 Core post text generated by Layer 2: '{core_post_text[:100]}...'")
    ...
    for platform_name, platform_post_type_from_map in platform_configs.items():
        print(f"⚙️ Preparing adaptation for {platform_name} with post type: {platform_post_type_from_map}")
    ...
    for platform_name, platform_output_data in platform_outputs_map.items():  # Changed variable name
        ...
        if current_platform_post_type_from_config in ["Image", "Video"]:
            ...
            if media_prompt:
                ...
                print(
                    f"🖼️ Scheduling {media_type_to_generate} generation for {platform_name} (Prompt: '{media_prompt[:50]}...')")
                ...
                print(
                    f"ℹ️ Info: Platform agent for {platform_name} (type: {current_platform_post_type_from_config}) did not provide a media prompt.")
        else:
            print(
                f"ℹ️ Info: Platform {platform_name} is Text-only (type: {current_platform_post_type_from_config}). No media generation needed.")
        ...
        try:
            ...
            for i, res_or_exc in enumerate(results_or_exceptions):
                ...
                if isinstance(res_or_exc, Exception):
                    print(f"🚨 Error generating media for {platform_name_for_media}: {res_or_exc}")
                    generated_media_paths.append(None)
                else:
                    generated_media_paths.append(res_or_exc)
        except Exception as e:
            print(f"🚨 Major error during media generation gathering: {e}")
        ...
        if media_info_for_platform:
            ...
            if media_path_idx < len(generated_media_paths):
                ...
                if path_or_none:
                    ...
                    print(f"⚠️ Media generation failed or skipped for {platform_name}. No media asset will be linked.")
                media_path_idx += 1
            else:
                print(f"🔍 Warning: Mismatch in media paths and platforms needing media for {platform_name}.")
...
async def main():
    ...
    print("\n" + "=" * 60)
    print("DODOOOOOOOOOOOOOOOOOOOOOOOOOOOOO 📊 PIPELINE EXECUTION SUMMARY")
    print("=" * 60)
            ...
            if isinstance(cloud_info, dict) and 'uploads' in cloud_info:  # Check structure
                for upload in cloud_info.get('uploads', []):
                    if isinstance(upload, dict) and upload.get('success'):  # Check structure
                        ...
                        print(f"  ❓ Unknown cloud upload item format: {upload}")
            else:
                print(f"  ❓ Cloud storage info format unexpected: {cloud_info}")
    ...
    if pipeline_result.get('summary_cloud_storage'):
        ...
        if isinstance(summary_cloud, dict) and summary_cloud.get('success'):  # Check structure
            ...
            print(f"  ❓ Summary cloud storage info format unexpected: {summary_cloud}")
...
async def generate_enhanced_social_media_posts_pipeline(
    ...
    for platform in selected_platforms:
        ...
        if effective_control and effective_control.enabled and platform.post_type in ["Image", "Video"]:
            ...
            # Aspect ratio is passed as a parameter to the model, NOT in the prompt
            # The ratio will be extracted in get_image_generation_config() and passed via image_config
